def orion():
    """This method goes through a typical PNICER session and creates an
    extinction map of Orion A from 2MASS data."""
    ...
    with fits.open(science_path) as science, fits.open(control_path) as control:
        ...
        # Coordinates
        sci_coo = SkyCoord(
            l=sci["GLON"], b=sci["GLAT"], frame="galactic", equinox="J2000", unit="deg"
        )
        con_coo = SkyCoord(
            l=con["GLON"], b=con["GLAT"], frame="galactic", equinox="J2000", unit="deg"
        )

        # Photometry
        sci_phot, con_phot = [sci[n] for n in feature_names], [
            con[n] for n in feature_names
        ]
    ...
    # Initialize pnicer
    science = ApparentMagnitudes(
        magnitudes=sci_phot,
        errors=sci_err,
        extvec=feature_extinction,
        coordinates=sci_coo,
        names=feature_names,
    )
    control = ApparentMagnitudes(
        magnitudes=con_phot,
        errors=con_err,
        extvec=feature_extinction,
        coordinates=con_coo,
    ...
    # Make extinction map
    pnicer_emap = ext_pnicer.build_map(
        bandwidth=5 / 60, metric="gaussian", nicest=False, use_fwhm=True
    )
    ...
    print(
        "{0:<80s}".format(
            "PNICER routines terminated successfully! Happy hunting for extinction :) "
        )
    )
